...
const C = {
  ...
  green: '#22C55E', red: '#EF4444', amber: '#F7931E', purple: '#AB47BC', blue: '#38BDF8',
}
...
function IndexStrip({ regime }: { regime: RegimeState | null }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'stretch', background: '#090909', borderBottom: `1px solid ${C.border}`, flexShrink: 0, height: 58 }}>
      <div style={{ padding: '0 16px', borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 2, minWidth: 86 }}>
        <span style={{ fontSize: 10, fontWeight: 700, color: C.text, letterSpacing: '.1em' }}>F&amp;O</span>
        <span style={{ fontSize: 8, fontWeight: 700, color: C.purple, letterSpacing: '.1em', background: '#15081A', border: `1px solid ${C.purple}33`, borderRadius: 3, padding: '1px 6px' }}>DERIVATIVES</span>
      </div>
      {INDICES.map(({ token, label, lot }) => {
        ...
        return (
          <div key={token} style={{ flex: 1, padding: '0 14px', borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 2 }}>
            <span style={{ fontSize: 8, color: C.dim, letterSpacing: '.08em' }}>{label} <span style={{ color: '#2A2A2A' }}>· LOT {lot}</span></span>
            ...
            <span style={{ fontSize: 9, color: chg >= 0 ? C.green : C.red, fontVariantNumeric: 'tabular-nums' }}>
      ...
      <div style={{ flex: 1, padding: '0 14px', borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 2 }}>
        <span style={{ fontSize: 8, color: C.dim, letterSpacing: '.08em' }}>INDIA VIX</span>
        ...
        <span style={{ fontSize: 8, color: C.muted }}>{vix > 25 ? 'elevated — size down' : vix > 18 ? 'watchful' : 'calm'}</span>
      </div>
      <div style={{ flex: 1.2, padding: '0 14px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 2 }}>
        <span style={{ fontSize: 8, color: C.dim, letterSpacing: '.08em' }}>REGIME · SIZE MULT</span>
        ...
        <span style={{ fontSize: 8, color: C.muted }}>drives derivative position sizing</span>
      </div>
    </div>
  )
}
...
function IndexSignals({ results }: { results: OrchestratorResult[] }) {
  ...
  return (
    <div className="panel" style={{ borderRadius: 5 }}>
      <div className="panel-header">INDEX STRATEGY SIGNALS <span style={{ color: '#2A2A2A' }}>S1 ORB · S2 52W · S8 VIX FADE</span></div>
      <div className="panel-body" style={{ padding: 10 }}>
        {indexResults.length === 0 ? (
          <div style={{ fontSize: 10, color: C.muted, padding: 20, textAlign: 'center' }}>
          ...
          <div key={r.token} style={{
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: C.text, minWidth: 90 }}>{r.symbol}</span>
            <span style={{ fontSize: 10, fontWeight: 700, color: r.side === 'BUY' ? C.green : r.side === 'SELL' ? C.red : C.muted, minWidth: 70 }}>{r.verdict}</span>
            <span style={{ fontSize: 10, color: C.sub, fontVariantNumeric: 'tabular-nums' }}>{r.price.toLocaleString('en-IN')}</span>
            <span style={{ fontSize: 9, color: C.muted }}>RSI {r.rsi.toFixed(0)}</span>
            {r.ev > 0 && <span style={{ fontSize: 9, color: C.amber }}>EV {r.ev.toFixed(2)}</span>}
            <span style={{ fontSize: 9, color: C.muted, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.summary}</span>
          </div>
        ))}
      </div>
    </div>
...
function SignalLog({ signals }: { signals: SignalRec[] }) {
  return (
    <div className="panel" style={{ borderRadius: 5 }}>
      <div className="panel-header">SIGNAL LOG <span style={{ color: '#2A2A2A' }}>INDEX COMPLEX</span></div>
      <div className="panel-body">
        {signals.length === 0 ? (
          <div style={{ fontSize: 10, color: C.muted, padding: 20, textAlign: 'center' }}>No recent index signals.</div>
        ) : (
          <table>
      ...
      <div className="panel-body" style={{ padding: 10 }}>
        ...
        {items.map(i => (
          <div key={i.title} style={{ display: 'flex', gap: 10, padding: '7px 0', borderTop: `1px solid ${C.border}`, alignItems: 'baseline' }}>
            <span style={{ fontSize: 8, fontWeight: 700, color: i.phase === 'P2' ? C.amber : C.muted, border: `1px solid ${i.phase === 'P2' ? C.amber + '44' : C.border2}`, borderRadius: 3, padding: '1px 6px', flexShrink: 0 }}>{i.phase}</span>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.text }}>{i.title}</div>
              <div style={{ fontSize: 9, color: C.muted, lineHeight: 1.5 }}>{i.desc}</div>
            </div>
          </div>
        ))}
      </div>
