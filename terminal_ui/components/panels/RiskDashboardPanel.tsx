...
const REGIMES = [
  ...
  { name: 'sideways',    label: 'SIDEWAYS',    color: '#FFB02E', mult: 0.7, desc: 'Range-bound' },
  ...
  return (
    <div style={{ height: '100%', background: '#0A0B0D', display: 'flex', alignItems: 'center', overflow: 'hidden', gap: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 9, color: '#4A4F57', letterSpacing: '.09em' }}>REGIME</span>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
          <span style={{
            fontSize: 13, fontWeight: 800, letterSpacing: '.04em',
            color: currentDef?.color ?? '#AEB3BB',
          }}>
            {currentDef?.label ?? current.replace(/_/g, ' ').toUpperCase()}
          </span>
          {conf !== null && (
            <span style={{ fontSize: 9.5, color: '#71767F', fontVariantNumeric: 'tabular-nums' }}>{conf}%</span>
          )}
        </div>
      </div>
      ...
      <div style={{ padding: '0 14px', height: '100%', flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 2, borderRight: '1px solid #23272E' }}>
        <span style={{ fontSize: 9, color: '#4A4F57', letterSpacing: '.09em' }}>SIZE ×</span>
        <span style={{ fontSize: 13, fontWeight: 700, color: currentDef?.color ?? '#AEB3BB', fontVariantNumeric: 'tabular-nums' }}>
          {sizeMul.toFixed(2)}
        </span>
      </div>
      <div style={{ padding: '0 14px', height: '100%', flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 2, borderRight: '1px solid #23272E' }}>
        <span style={{ fontSize: 9, color: '#4A4F57', letterSpacing: '.09em' }}>INDIA VIX</span>
        <span style={{ fontSize: 13, fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: vix > 25 ? '#EF5350' : vix > 18 ? '#FFB02E' : '#AEB3BB' }}>
          {vix > 0 ? vix.toFixed(2) : '—'}
        </span>
      </div>
            ...
            <div key={r.name} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <span style={{
                  ...
                  background: active ? r.color : '#4A4F57',
                  boxShadow: active ? `0 0 5px ${r.color}88` : 'none',
                  transition: 'all .2s',
                }} />
                <span style={{ fontSize: active ? 10 : 8, fontWeight: active ? 800 : 500, color: active ? r.color : '#4A4F57', letterSpacing: '.04em', whiteSpace: 'nowrap' }}>
                  {r.label}
                </span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingLeft: 11 }}>
                <span style={{ fontSize: 9.5, color: active ? `${r.color}99` : '#333841', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>
                  size ×{r.mult}
                </span>
                <span style={{ fontSize: 9, color: active ? '#71767F' : '#23272E', whiteSpace: 'nowrap' }}>
                  {r.desc}
                </span>
              </div>
            </div>
