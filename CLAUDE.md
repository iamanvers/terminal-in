...
terminal_in/                        ← Python backend
  ...
  strategy_engine/                  — engine (8 active S1–S9 ex S7; S6 = single-leg
                                      relative-value, hedge leg is roadmap), DSA, regime/ (HMM)
...
**Settlement mechanics** — product-aware like a real broker: MIS (S1, time_exit, explicit, **and ANY cash SELL/short** — NSE has no overnight CNC delivery short, so shorts are intraday-only; `_product_for`) squares off at EOD (`mis_square_off`); CNC (default, long delivery) carries overnight and exits ONLY when ticks cross stop-loss/target (`paper_broker._check_exit`). EOD snapshot marks carried positions to market. Signals: S2–S9 once per (strategy, token) per session; S1 30-min cooldown.
...
**F&O execution (P2, Stages 1–5)** — derivatives get their OWN path (the cash path would corrupt risk checks). `data_ingest/fno_instruments.py` = contract model (synthetic deterministic tokens ≥9e11, expiry calendar, chain builder); `execution/options_pricing.py` = pure-stdlib Black-Scholes (price + greeks). **DATA HONESTY:** option premiums/greeks are Black-Scholes **theoretical** from REAL spot + India VIX (the NIFTY 30d IV) as the **ATM IV anchor**, labeled `theoretical=True`; **OI/real-IV/volume are live-only and null in paper — never fabricated**, nothing written to `ohlcv_*`. `execution/vol_surface.py` = pure-stdlib **skew/smile surface** (`skew_iv`) that turns that flat ATM anchor into a strike-dependent IV (equity-index negative skew: OTM puts richer, OTM calls cheaper, mild wing smile, short-tenor steeper; clamped; ATM preserved bit-for-bit). Used by the chain builder AND `fno_paper_broker._iv_at` (one IV per leg → pricing+SPAN+greeks consistent). Coefficients are LABELED estimates (NIFTY-like, as-of 2026 — verify against a real surface); `VOL_SURFACE=false` reverts to flat-VIX exactly; live Kite per-strike IV replaces it. Chain payload carries `iv_model: 'skew'|'flat'`. `execution/fno_paper_broker.py` = lot-based paper execution (qty=lots×lot_size, entry_price=premium → (exit−entry)×qty IS premium P&L), marks positions by re-pricing on underlying ticks, expiry square-off at intrinsic, **shares the cash account** via `PaperBroker.reserve_capital/release_capital/apply_external_pnl`. `risk/span_margin.py` = scenario-based SPAN approximation (worst loss over price ±3.5σ/2-day VIX-implied × vol grid + 2% exposure; long option = premium paid). `execution/fno_signal_router.py` = S1/S8 index signals → ATM CE (BUY) / PE (SELL) on the F&O broker (market-hours + kill-switch checked). **Risk caps** (`_risk_check`): count caps (max positions/per-expiry/short-legs/margin) PLUS equity-normalized **portfolio greek caps** (net delta notional ≤400%, net short-gamma loss on a 2% gap ≤5%, net vega ≤2% of equity — composed over the prospective book) and **event-day limits** (full blackout on a 0-mask event; near expiry/RBI/FOMC refuse new short-gamma legs). Greek checks gate behind `leg_greeks` so the live order path runs them; the count/margin path is unchanged. `portfolio_greeks()` + per-leg greeks served on `/api/fno/positions`. API `/api/fno/{underlyings,expiries,chain,order,positions,close}`. Only paper mode; live Kite F&O is a later stage. Tests: `tests/test_fno*.py`.

**F&O multi-leg strategies (Stage 6)** — `fno_paper_broker.place_combo(legs, meta)` places a multi-leg structure **ATOMICALLY**: it `_quote_leg`s all legs, runs `_risk_check_combo` (caps on the COMBO AS A WHOLE — net greeks of all legs vs the prospective book, never leg-by-leg, so a balanced iron condor isn't rejected on its first short leg), reserves total margin once, then records every leg under one `combo_id` (persisted; trade_ids uuid-suffixed). `execution/fno_strategies.py` = pure leg-builders (`vertical_spread_legs` debit, `iron_condor_legs`, `short_strangle_legs`, `straddle_legs`, `covered_call_legs`, `futures_pair_legs` long-cheap/short-rich FUT) — strikes snap to the real NSE interval. `execution/fno_strategy_manager.py` (`FnOStrategyManager`, own thread, `FNO_MANAGER_INTERVAL_S` cadence) fires the triggered ones each scan with book-reconciled dedupe: **variance harvest** (range regime + VIX 12–22 → NIFTY iron condor — the documented variance-risk-premium; the greek/event caps were built for it), **futures pair** (cointegration |z|≥2 → long/short single-stock FUTURES, the only fundable market-neutral form), **covered call** (held cash long in an F&O name → sell OTM call), **event straddle** (event_cal mask 0<m<1 → long ATM straddle, vol-expansion). Per-kind env toggles `FNO_HARVEST/PAIR/COVERED_CALL/EVENT_STRADDLE`. The signal router now expresses S1/S8 directional views as **risk-defined debit spreads** by default (`FNO_DIRECTIONAL_STRUCTURE=spread|option`, `FNO_SPREAD_WIDTH`). All are eval-gated capabilities, NOT tuned alpha (ALPHA_FINDINGS: reversal/pairs edge dies under hardening). Tests: `tests/test_fno_strategies.py`, `tests/test_fno_manager.py`.

**F&O backtest (eval gate)** — `backtest/fno_engine.py` `run_fno_backtest(db, strategy='iron_condor', short_sd=1.0, wing=4, risk_frac)` replays the monthly NIFTY short iron condor over 10y of REAL NIFTY + India-VIX daily bars; shorts at the VIX-implied ~1-SD expected move (NOT fixed steps — fixed-step shorts sit ~0.75% OTM and lose 72% of months; the live `_fire_condor` now uses the same σ√T placement), realistic NSE-options cost estimate at entry+exit, no lookahead (strikes from entry close+VIX, realised at expiry close). **DATA HONESTY: premiums are BS THEORETICAL (real spot+VIX, no real tape) — this FLATTERS short-premium, so the result is an UPPER bound.** Verdict (ALPHA_FINDINGS #8): even the variance-risk-premium harvester is **not a deployable edge** — 75% win but CAGR +0.74% / Sharpe 0.13 / −25% DD theoretical, worse than cash. Served `POST/GET /api/backtest/fno` (synchronous, <1s). Tests: `tests/test_fno_backtest.py`. ⚠️ futures-pair/covered-call/straddle backtests not yet built (condor is the headline edge prior).
...
**Firm-knowledge plane (vector-less PIT RAG — `terminal_in/knowledge/`)** — the orthogonal-signal substrate: grounds the analyst/judge in firm reality with REAL firm text, after the validation harness proved price-only factors exhausted (ALPHA_FINDINGS, 9 negatives). `firm_store.py` = a single **SQLite-FTS5** table (BM25 lexical retrieval, **NO embeddings / no vector index** — the owner's "tensored-vector-less" RAG); one row per firm document `(symbol, doc_type, source, filing_date, period_end, title, body, summary, confidence, compacted)`. Three inherited invariants: **POINT-IN-TIME** (`retrieve(symbol, query, as_of)` filters filing_date ≤ as_of in-SQL — a decision dated D can never see a doc filed after D), **FAIL-CLOSED** (undatable / no-symbol / no-title rows dropped + counted, never guessed — mirrors `events.py`/`fundamentals.py`), **idempotent** (sha1 doc_id; re-ingest replaces). **Rolling 5-year horizon** (the owner's "compression + deleting timelines"): `compact()` keeps full text ≤ `KNOWLEDGE_RAW_DAYS` (400d), **compresses** the 13mo–5y band (drop body, keep searchable summary, `compacted=1`), and **purges** > `KNOWLEDGE_HORIZON_DAYS` (1826d). `ingest.py` = adapter protocol + `KnowledgeIngestor` thread (cadence `KNOWLEDGE_INGEST_INTERVAL_H`=24, publishes `knowledge.ingest`): **EventsAdapter** (REAL — NSE announcement archive → docs; event + filing date, not figures); **NewsAdapter** (activates when a news archive is persisted); **BseFilingsAdapter** (REAL — `data_ingest/bse_filings.py` pulls the BSE corporate-filings API with browser headers, fail-closed dating, graceful [] on block; symbol→scrip-code map in `BSE_SCRIP_CODES`, overridable via `data/knowledge/bse_scrip_codes.json`, unmapped symbols skipped not guessed); **IrPdfAdapter** (REAL firm-document DEPTH via `knowledge/pdf_extract.py` (pypdf): TWO offline-capable inputs — (1) the TURNKEY local folder `data/knowledge/ir_docs/` where you drop firm PDFs named `SYMBOL__YYYY-MM-DD__title.pdf` (or under `ir_docs/SYMBOL/`), and (2) `data/knowledge/ir_sources.json` `{symbol:[pdf_url]}` fetched if reachable; dates FAIL-CLOSED (filename date → PDF /CreationDate → Last-Modified → drop), doc_type inferred from title; no inputs = no-op). BSE attachment depth is opt-in via `KNOWLEDGE_FETCH_PDF=true` (`bse_filings.enrich_with_pdf`). **BSE direct API is bot-blocked from a plain client (returns the 'No Record Found!' sentinel) — confirmed; needs a real browser session or licensed feed, so firm depth comes today via the local `ir_docs/` folder.** Bot-hostile sources NEVER fabricate — they return [] and forward-accumulate. On-demand API: `POST /api/knowledge/ingest`, `GET /api/knowledge/{coverage,search}`. `rag.build_context(symbol, query, as_of, budget_chars)` assembles a compact citation-tagged (source · date · confidence-label) grounded block; empty-but-honest when nothing's on file (no hallucinated filler). Wired in `main.py` (`KNOWLEDGE_INGEST_ENABLED`), coverage on `/api/health.knowledge`. Store: `data/knowledge/firm_knowledge.db` (gitignored, `KNOWLEDGE_DB_PATH`). **WIRED into the AI analyst**: `financial_agent._firm_context()` detects NSE symbols in the question and injects a `FIRM CONTEXT` RAG block (alongside `LIVE CONTEXT`, via `_compose_context`); the system prompt tells the model to cite `[n]` and weight `filed` > `reported`/`unverified` — honest-empty before ingest fills, so it degrades to no-op cleanly. **Firm-research collector** (`data_ingest/firm_research.py`) — autonomous firm-intelligence gatherer: reads a firm's robots.txt + sitemap(s), ranks every URL against an **exhaustive `COLLECTION_SPEC`** (16 categories — annual_report/financial_results/investor_presentation/transcript/regulatory_disclosure/corp_action/M&A/management_change/press_release/product_launch/credit_rating/shareholding/agm_egm/governance/esg/business_profile — each with priority, doc_type, and a **cadence**: volatile|periodic|static), fetches the top docs (HTML pages also harvest their linked report PDFs — the actual reports aren't in the sitemap), extracts (pypdf/HTML-strip), dates POINT-IN-TIME (sitemap `<lastmod>`→PDF `/CreationDate`→`Last-Modified`, FAIL-CLOSED drop), and stores CHUNKED. **Two-phase cadence**: `profile_firm` (heavy, periodic/weekly) crawls the whole sitemap, maps URL→category, persists a **per-ticker profile** (`data/knowledge/profiles/<SYM>.json`) + initial ingest; `refresh_firm` (cheap, every session) re-checks only **volatile/periodic** links (news/updates/results — NOT annual reports) and ingests deltas (new or newer-`lastmod`). `run_ingest` auto-runs refresh for already-profiled firms (inert until profiled). Fetch via **curl_cffi** browser TLS profile to clear Akamai/Cloudflare WAFs (plain requests AND BSE/HUL both 403 a naive client — confirmed); `FIRM_DOMAINS` seed + `firm_domains.json` override, **unmapped firm skipped (domain never guessed)**. API: `POST /api/knowledge/research` (profile), `POST /api/knowledge/research/refresh` (deltas), `GET /api/knowledge/{profile,spec}`. **Chunking**: `firm_store.record_documents(chunk_chars=)` splits long bodies into overlapping FTS rows (`title [i/n]`) so heterogeneous firm docs retrieve at chunk granularity (default on; short docs unaffected). Tests: `test_firm_knowledge.py` + `test_bse_filings.py` + `test_firm_research.py` (sitemap per-block dating, cadence, profile→refresh deltas, chunking; mocked fetcher — no network). NEXT: LLM-summarised compression; feed the RAG into the M6 relational plane; expand `FIRM_DOMAINS`/`bse_scrip_codes.json`.
...
- 72-symbol universe with full sector coverage; real-data-only ingest; degraded-mode surfacing; 301 tests passing
